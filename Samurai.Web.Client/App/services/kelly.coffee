...
class WhitrowKelly extends Kelly
    ...
    noBets              : -> @calculatedBets.length if @calculatedBets
    singleKellyStakes   : -> @normaliseKelly(@calcSingleKellyStakes(@calculatedBets, 1))
...
class ExhaustiveKelly extends Kelly
    ...
    parlaySize : (n) ->
        @toRadix(n, 2).split('').reduce (t, s) -> t or 0 + s
    ...
    calculateKelly : ->
        ...
        for i in [1..singles] by 1
            parlayMaps[i] = []
        
        for i in [1..bets] by 1
            pSize = parseInt(@parlaySize(i))
            parlayMaps[pSize - 1] = [] unless parlayMaps[pSize - 1]
