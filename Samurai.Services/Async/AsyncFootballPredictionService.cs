...
namespace Samurai.Services.Async
{
  ...
  public class AsyncFootballPredictionService : AsyncPredictionService, IAsyncFootballPredictionService
  {
    ...
    public async Task<IEnumerable<FootballPredictionViewModel>> GetFootballPredictionsNoScorePredictions(IEnumerable<FootballFixtureViewModel> fixtures)
    {
      var predictions = await Task.Run(() => GetFootballPredictionsFromFixtures(fixtures, false));
    ...
    private IEnumerable<FootballPrediction> GetFootballPredictionsFromFixtures(IEnumerable<FootballFixtureViewModel> fixtures, bool getScorePredictions = true)
    {
      ...
      var scoreLinePredictions = getScorePredictions ? this.predictionRepository.GetScoreOutcomeProbabilitiesInMatchByIDs(matchIDs.Keys) : new Dictionary<int, List<ScoreOutcomeProbabilitiesInMatch>>();
      ...
      foreach (var id in matchIDs.Keys)
      {
        ...
        if (scoreLinePredictions.Count > 0)
